...
impl Env {
    async fn new() -> Self {
        ...
        let tx = Transaction::new_signed_with_payer(
            &[SmartCounterInstruction::update_settings(
                &admin.pubkey(),
        ...
        let ix = system_instruction::create_account_with_seed(
            &user.pubkey(),
            &SmartCounter::get_smart_counter_pubkey(&user.pubkey()),
            &user.pubkey(),
            COUNTER_SEED,
            lamports,
            space as u64,
...
async fn test_inc() {
    ...
    let tx = Transaction::new_signed_with_payer(
        &[SmartCounterInstruction::inc(&env.user.pubkey())],
    ...
    let acc = env
        .ctx
        .banks_client
        .get_account(SmartCounter::get_smart_counter_pubkey(&env.user.pubkey()))
        .await
        .unwrap()
        .unwrap();

    let counter = SmartCounter::try_from_slice(acc.data.as_slice()).unwrap();
...
async fn test_dec() {
    ...
    let tx = Transaction::new_signed_with_payer(
        &[SmartCounterInstruction::dec(&env.user.pubkey())],
        Some(&env.user.pubkey()),
        &[&env.user],
        env.ctx.last_blockhash,
    );
